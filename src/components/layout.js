...
import '../css/cf-framework.min.css'
import {
  Contain,
  Wrapper,
  Content,
...
const ToolbarRenderer = ({ sidebarClickHandler, isSidebarVisible }: Object) =>
  isSidebarVisible ? null : (
    <Toolbar className={isSidebarVisible ? 'visible' : null}>
      ...
      <ToolbarPushWrapper>
        <ToolbarIcon src={favicon} alt="Christex Foundation" />
      </ToolbarPushWrapper>
    </Toolbar>
  )
      ...
      {({ location }) => (
        <Sidebar>
          <SidebarReset>
            <SidebarScroll>
              <ScrollbarHeader>
                <SidebarIcon src={favicon} alt="Christex Foundation" />
                <FlexboxPush>
...
class Layout extends React.Component<Props, State> {
  ...
  render() {
    ...
    return (
      <Contain>
        <Helmet>
          <title>Progression at Christex Foundation</title>
        </Helmet>
        <SidebarRenderer
          data={data}
